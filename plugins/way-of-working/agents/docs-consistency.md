...
description: >-
  Opus read-only auditor that cross-checks loop-orchestrator's load-bearing prose
  (CLAUDE.md, docs/migration_roadmap.md, docs/backlog.md, .ai/context/*, sprint plans)
  against ground truth (the code, the tests, the CI/ruleset config) and reports
  contradictions and stale claims — the "correct a false claim" (#64) / BL-32 failure mode,
  caught systematically. Read-only: returns a ranked findings list, never edits docs. Its
  core skill is telling a genuine contradiction from intentional historical/aspirational
  prose — it must not flag the latter.
model: opus
